...
class Usuario:
    ...
    def from_dict(cls, data):
        return cls(data["documento"], data["nombre"], data["password"])
...
class Cita:
    ...
    def from_dict(cls, data):
        ...
        if tipo == "Médico general":
            return CitaMedico(data["fecha"], data["hora"], data["sede"], data.get("motivo", ""))
        elif tipo == "Psicología":
            return CitaPsicologia(data["fecha"], data["hora"], data["sede"], data.get("sesion", ""))
        elif tipo == "Toma de muestras":
            return CitaMuestras(data["fecha"], data["hora"], data["sede"], data.get("tipo_muestra", ""))
        elif tipo == "Odontología":
            return CitaOdontologia(data["fecha"], data["hora"], data["sede"], data.get("procedimiento", ""))
        elif tipo == "Especialista":
            return CitaEspecialista(data["fecha"], data["hora"], data["sede"], data.get("especialidad", ""))
        return cls(data["fecha"], data["hora"], data["sede"])
...
class CitaMuestras(Cita):
    ...
    def to_dict(self):
        data = super().to_dict()
        data["tipo_muestra"] = self.tipo_muestra
        return data
...
class CitaOdontologia(Cita):
    ...
    def to_dict(self):
        data = super().to_dict()
        data["procedimiento"] = self.procedimiento
        return data
...
class SistemaEPS:
    ...
    def cargar_datos(self):
        if not os.path.exists(self.ARCHIVODATOS):
            return
        try:
            with open(self.ARCHIVODATOS, "r", encoding="utf-8") as f:
                data = json.load(f)
                for doc, user_data in data.get("usuarios", {}).items():
                    self.usuarios[doc] = Usuario.from_dict(user_data)
                for doc, citas_list in data.get("citas", {}).items():
                    self.citas[doc] = [Cita.from_dict(c) for c in citas_list]
    ...
    def guardar_datos(self):
        try:
            data = {
                "usuarios": {doc: u.to_dict() for doc, u in self.usuarios.items()},
                "citas": {doc: [c.to_dict() for c in clist] for doc, clist in self.citas.items()}
            }
            with open(self.ARCHIVODATOS, "w", encoding="utf-8") as f:
    ...
    def obtener_estadisticas(self):
        ...
        for lista_citas in self.citas.values():
            total_citas += len(lista_citas)

            for cita in lista_citas:
                tipo = cita.tipo()

                if tipo in citas_por_tipo:
                    citas_por_tipo[tipo] += 1

                sede = cita.sede

                if sede not in citas_por_sede:
                    citas_por_sede[sede] = 0

                citas_por_sede[sede] += 1

        sede_mas_solicitada = "No hay citas registradas"

        if citas_por_sede:
            sede_mas_solicitada = max(
                citas_por_sede,
                key=citas_por_sede.get
            )
        ...
        return {
            ...
            "citas_por_sede": citas_por_sede,
            "sede_mas_solicitada": sede_mas_solicitada
        }
...
class VentanaMenuUsuario(tk.Toplevel):
    ...
    def _build(self):
        ...
        btns = [
            ...
            ("📊  Estadísticas",      self._estadisticas,  "#7C3AED"),
    ...
    def _estadisticas(self):
        VentanaEstadisticas(self, self.sistema)
...
class VentanaEstadisticas(tk.Toplevel):
    ...
    def _build(self):
        ...
        # Tarjeta principal
        card = card_frame(self, padx=25, pady=20)
        ...
        tk.Label(
            card,
            text=f"👤 Total de usuarios registrados: {estadisticas['total_usuarios']}",
            font=FONT_HEAD,
            bg=CARD,
            fg=TEXT
        ...
        tk.Label(
            card,
            text=f"📅 Total de citas registradas: {estadisticas['total_citas']}",
            font=FONT_HEAD,
            bg=CARD,
            fg=TEXT
        ).pack(anchor="w", pady=8)
        ...
        tk.Label(
            card,
            text=estadisticas["sede_mas_solicitada"],
            font=FONT_BODY,
            bg=CARD,
            fg=TEXT,
            wraplength=520,
        ...
        tk.Label(
            card,
            text="📋 Cantidad de citas por tipo:",
            font=FONT_HEAD,
            bg=CARD,
            fg=PRIMARY
        ).pack(anchor="w", pady=(20, 8))

        for tipo, cantidad in estadisticas["citas_por_tipo"].items():
            tk.Label(
                card,
